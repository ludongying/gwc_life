  ...
  L.Control.NavBar = L.Control.extend({
    ...
    onAdd: function(map) {
      ...
      var controlName = 'leaflet-control-navbar',
          container = L.DomUtil.create('div', controlName + ' leaflet-bar',container2);

      // Add toolbar buttons
      this._homeButton = this._createButton(options.homeTitle, controlName + '-home', container, this._goHome);
      var className = 'leaflet-control-pan1';
      var container1 = L.DomUtil.create('div', className,container2);
      // this._panButton('Left' , className + '-left',
      // 				container1, this._goFwd);
      // this._panButton('Right', className + '-right',
      // 				container1, this._goBack);
      //    var classname = L.DomUtil.create('div', controlName + '-setsize', container);
      this._fwdButton = this._createButton(options.forwardTitle, controlName + '-fwd', container1, this._goFwd);
      this._backButton = this._createButton(options.backTitle, controlName + '-back', container1, this._goBack);
